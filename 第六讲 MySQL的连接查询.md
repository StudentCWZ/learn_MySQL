...
```
2. left join /left outer join (左外连接)：左外连接，会把左边表里面的所有数据取出来，而右边表中的数据，如果有相等，就显示出来，如果没有，就会补NULL。
```
...
3 rows in set (0.00 sec)
```
3. right join /right outer join(右外连接)：右外连接，会把右边表里面的所有数据取出来，而左边表中的数据，如果有相等，就显示出来，如果没有，就会补NULL。
```
